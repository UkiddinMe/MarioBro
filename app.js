...
function gameLoop() {
    ...
    // requestAnimationFrame è una funzione javascript che dice al browser che vuoi fare un'animazione.
    // Chiede al browser di eseguire una data funzione di callback PRIMA del prossimo repaint.
    // E' one-shot, quindi va richiamata
...
function update()
{
    ...
    for (let pipe of gameObjects.pipes) {
        if (checkCollision(player, pipe)) {
            if (player.velocityY > 0) { // Fallin down onto Pipe
    ...
    for (let enemy of gameObjects.enemies) {
        ...
        if (!onPlatform || enemy.x <= 0 || enemy.x >= 800) {
            enemy.direction *= -1 //inverti il valore da positivo o negativo e vice versa
        }
        ...
        if (checkCollision(player, enemy)) {
            if (player.velocityY > 0 && player.y < enemy.y) { // je stai saltando sopra
                enemy.alive = false
